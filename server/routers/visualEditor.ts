...
const supportedElementSchema = z.string().min(3).max(128).regex(/^(?:home|school|event|events|lobby|dashboard|workspace|scan|control|attendees|operations|reports|settings|audit|roles|custom|page|section|journal|news|album|albums|showcase|offers|studio|aqeeq|live|blueprint|stage|memory|maison|guest|about|admissions|accreditations|app|auto)-[a-z0-9-]+$/, "العنصر غير مدعوم في المحرر البصري");

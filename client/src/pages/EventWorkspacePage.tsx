...
export default function EventWorkspacePage({ id }: { id?: string }) {
  ...
  const phaseTone = experience.phase === "live" ? "text-sky-200 border-sky-400/35 bg-sky-400/10" : experience.phase === "ready" ? "text-emerald-200 border-emerald-400/35 bg-emerald-400/10" : experience.phase === "archived" ? "text-slate-300 border-slate-600 bg-slate-700/30" : "text-amber-200 border-amber-400/35 bg-amber-400/10";
    ...
    <main dir="rtl" className="aq-admin-surface min-h-screen w-full overflow-x-hidden text-slate-100" style={{ fontFamily: event.fontFamily || "Tajawal" }}>
      ...
      <div className="container py-7 md:py-9">
        {activeTab === "overview" && <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><WorkspaceStat icon={Users} label="إجمالي الضيوف" value={experience.total} color="text-amber-400" /><WorkspaceStat icon={CheckCircle2} label="تم تسجيل دخولهم" value={experience.attended} color="text-emerald-400" /><WorkspaceStat icon={BarChart3} label="نسبة الحضور" value={`${experience.attendanceRate}%`} color="text-sky-400" /><WorkspaceStat icon={QrCode} label="تذاكر مدفوعة" value={experience.paid} color="text-violet-300" /></div>

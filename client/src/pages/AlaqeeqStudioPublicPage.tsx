...
// Dynamic Stories Data (Supporting All Content Types + Custom Selected Stories)
  ...
  const { scrollYProgress: weeklyProgress } = useScroll({
    target: weeklySectionRef,
    offset: ["start end", "end start"],
  });

  const rawWeekly1Y = useTransform(weeklyProgress, [0, 1], [30, -30]);
  const rawWeekly2Y = useTransform(weeklyProgress, [0, 1], [15, -45]);
  const rawWeekly3Y = useTransform(weeklyProgress, [0, 1], [45, -20]);
  ...
  const wt1 = useMagneticTilt();
  const wt2 = useMagneticTilt();
  const wt3 = useMagneticTilt();
  ...
  return (
    ...
    >
      ...
      {/* 🌟 2. لوحة «بينتو إنجازات وأحداث الأسبوع التفاعلية» (Interactive 3D Bento Grid) */}
      <div ref={weeklySectionRef} className="relative w-full">
        <VisualEditable
        ...
        >
          <div className="mx-auto max-w-[1340px] px-5 md:px-8">
            ...
            <div className="grid gap-6 md:grid-cols-3 lg:grid-cols-4 items-stretch">
              ...
              <motion.div
                ref={wt1.ref}
                onMouseMove={wt1.onMove}
                onMouseLeave={wt1.onLeave}
                style={{
                  y: weeklyCard1Y,
                  rotateX: wt1.tilt.x,
                  rotateY: wt1.tilt.y,
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background: `radial-gradient(circle 260px at ${wt1.tilt.gx}% ${wt1.tilt.gy}%, rgba(248,202,20,0.14), transparent 70%)`,
                  }}
                />
                <VisualEditable
              ...
              <motion.div
                ref={wt2.ref}
                onMouseMove={wt2.onMove}
                onMouseLeave={wt2.onLeave}
                style={{
                  y: weeklyCard2Y,
                  rotateX: wt2.tilt.x,
                  rotateY: wt2.tilt.y,
              ...
              >
                ...
                <div
                  className="pointer-events-none absolute inset-0 rounded-[2.2rem] z-10 transition-opacity duration-300 opacity-0 group-hover:opacity-100"
                  style={{
                    background: `radial-gradient(circle 200px at ${wt2.tilt.gx}% ${wt2.tilt.gy}%, rgba(248,202,20,0.16), transparent 70%)`,
                  }}
                />
                <VisualEditable
              ...
              <motion.div
                ref={wt3.ref}
                onMouseMove={wt3.onMove}
                onMouseLeave={wt3.onLeave}
                style={{
                  y: weeklyCard3Y,
                  rotateX: wt3.tilt.x,
                  rotateY: wt3.tilt.y,
              ...
              >
                ...
                <div
                  className="pointer-events-none absolute inset-0 rounded-[2.2rem] z-10 transition-opacity duration-300 opacity-0 group-hover:opacity-100"
                  style={{
                    background: `radial-gradient(circle 200px at ${wt3.tilt.gx}% ${wt3.tilt.gy}%, rgba(222,25,30,0.16), transparent 70%)`,
                  }}
                />
                <VisualEditable

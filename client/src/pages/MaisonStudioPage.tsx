...
export default function MaisonStudioPage({ ceremonyId }: { ceremonyId: number }) {
  ...
  return <section dir="rtl" className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <section className="rounded-3xl border border-white/[0.08] bg-black/20 p-5 md:p-6"><div className="flex items-center gap-3"><Archive className="text-violet-300" size={20} /><div><p className="text-xs font-black text-violet-200">بورتريه المناسبة</p><h3 className="mt-1 text-xl font-black text-slate-100">الصورة التي تبقى بعد النهاية</h3></div></div><div className="mt-5 space-y-4"><Field label="الاقتباس الختامي" value={draft.portraitQuote} onChange={(value) => updateField("portraitQuote", value)} textarea /><Field label="لحظات مختارة — لحظة في كل سطر" value={draft.portraitHighlights} onChange={(value) => updateField("portraitHighlights", value)} textarea placeholder="لحظة الافتتاح\nتكريم المشاركين\nصورة الختام" /><Field label="رابط فيلم الختام (اختياري)" value={draft.portraitVideoUrl} onChange={(value) => updateField("portraitVideoUrl", value)} dir="ltr" placeholder="https://..." /></div></section>
    </div>

    <section className="rounded-3xl border border-white/[0.08] bg-black/20 p-5 md:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><Theater className="text-sky-300" size={21} /><div><p className="text-xs font-black text-sky-200">شاشة المسرح</p><h3 className="mt-1 text-xl font-black text-slate-100">تحكم كشف الستار الحي</h3></div></div><a href={`/event/${ceremonyId}/stage`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl border border-sky-300/25 px-4 py-2.5 text-xs font-black text-sky-100 hover:bg-sky-300/[0.08]"><Play size={15} />فتح شاشة المسرح</a></div><div className="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr_auto]"><Field label="عنوان الكشف" value={draft.curtainTitle} onChange={(value) => updateField("curtainTitle", value)} /><Field label="وصف اللحظة" value={draft.curtainSubtitle} onChange={(value) => updateField("curtainSubtitle", value)} /><div className="flex items-end"><div className="grid w-full grid-cols-3 overflow-hidden rounded-xl border border-slate-700">{(["closed", "opening", "revealed"] as const).map((state) => <button key={state} onClick={() => setCurtain(state)} className={`px-2 py-3 text-[11px] font-black transition ${draft.curtainState === state ? "bg-sky-300 text-slate-950" : "bg-black/20 text-slate-400 hover:text-white"}`}>{state === "closed" ? "مغلق" : state === "opening" ? "يفتح" : "انكشف"}</button>)}</div></div></div></section>

    ...
    <div className="mx-auto max-w-[1400px]">
      {view === "scroll" ? <section className="aq-reader-gold-frame mx-auto max-w-[1180px] overflow-auto p-3 pb-10 md:p-7"><div className="mx-auto space-y-8 transition-[width] duration-200" style={{ width: `${scrollZoom * 100}%`, minWidth: scrollZoom > 1 ? "820px" : undefined }}>{pages.map((item, itemIndex) => <article id={`aq-pager-scroll-page-${itemIndex}`} key={item.id} className="overflow-hidden rounded-sm bg-transparent shadow-[0_20px_60px_rgba(0,0,0,.35)]"><img loading="lazy" src={item.imageUrl} alt={item.caption || `الصفحة ${itemIndex + 1}`} className="block h-auto w-full" /></article>)}</div></section> : <section className="aq-reader-gold-frame px-3 py-7 md:px-16 md:py-12 touch-pan-y select-none" onPointerDown={handlePointerDown} onPointerUp={handlePointerUp} onPointerCancel={handlePointerCancel} onPointerLeave={handlePointerCancel}>
        <div className="relative mx-auto flex min-h-[480px] max-w-[590px] items-center justify-center gap-1 md:min-h-[680px] cursor-grab active:cursor-grabbing select-none">
          ...
          <article className="relative w-full overflow-hidden rounded-md bg-transparent shadow-[0_28px_75px_rgba(0,0,0,.55)]"><img loading="lazy" key={page.id} src={page.imageUrl} alt={page.caption || `صفحة ${index + 1}`} className="block h-auto w-full [animation:journal-in_.28s_cubic-bezier(.23,1,.32,1)]" /></article>
        ...
        <div className="relative mx-auto mt-5 flex max-w-[950px] gap-2 overflow-x-auto pb-2">{pages.map((item, itemIndex) => <button key={item.id} onClick={() => changePage(itemIndex)} className={`h-20 w-14 shrink-0 overflow-hidden rounded-sm border transition ${itemIndex === index ? "border-amber-300 ring-2 ring-amber-300/20" : "border-white/[.12] opacity-55 hover:opacity-100"}`}><img loading="lazy" src={item.imageUrl} alt={`صفحة ${itemIndex + 1}`} className="h-full w-full object-cover" /></button>)}</div>
      </section>}
    </div>
  </div>;
}

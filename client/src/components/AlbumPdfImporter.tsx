...
export default function AlbumPdfImporter({ onImported }: Props) {
  ...
  return <div className="space-y-2"><input ref={inputRef} type="file" accept="application/pdf,.pdf" className="hidden" onChange={importPdf} /><button type="button" onClick={() => inputRef.current?.click()} disabled={Boolean(status)} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#08467d]/40 bg-[#08467d]/10 px-3 py-2.5 text-xs font-black text-[#f8ca14] transition hover:bg-[#08467d]/20 disabled:opacity-55">{status ? <Loader2 size={15} className="animate-spin" /> : <FileUp size={15} />}{status || "رفع PDF وتحويله لصور"}</button>{status ? <div className="rounded-lg border border-[#08467d]/20 bg-[#08467d]/5 p-2"><div className="mb-1 flex justify-between text-[9px] text-amber-100"><span>{status}</span><span>{percent}%</span></div><Progress value={percent} className="h-1.5 bg-slate-800" /></div> : null}</div>;
}

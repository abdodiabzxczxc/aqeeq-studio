...
function PalettePreview({ item }: { item: PaletteItem }) {
  if (item.type === "image" || item.type === "video") return <div className={`mb-2 flex h-10 items-center justify-center rounded-lg ${item.type === "video" ? "bg-[#08467d]/20 text-[#f8ca14]" : "bg-amber-400/10 text-amber-300"}`}>{item.type === "video" ? <PlayCircle size={19} /> : <ImageIcon size={19} />}</div>;
...
export default function VisualFreeformCanvas({ pagePath }: { pagePath: string }) {
  ...
  const renderElementBody = (item: CanvasItem) => {
    ...
    if (item.elementType === "image") {
      return content.mediaUrl ? <img loading="lazy" src={content.mediaUrl} alt={content.altText || "صورة"} draggable={false} className={`h-full w-full shadow-2xl ${item.preset === "circle" ? "rounded-full" : "rounded-2xl"}`} style={{ objectFit: content.objectFit || "cover" }} /> : <div className={`flex h-full w-full items-center justify-center border border-dashed border-amber-300/55 bg-amber-400/[0.08] text-amber-200 ${item.preset === "circle" ? "rounded-full" : "rounded-2xl"}`}><span className="text-center"><ImageIcon className="mx-auto" size={30} /><span className="mt-2 block text-xs font-black">اختر صورة</span></span></div>;
    }
    if (item.elementType === "video") {
      return content.mediaUrl ? isAqeeqDriveVideo(content.mediaUrl) ? <AqeeqVideoPoster sourceUrl={content.mediaUrl} posterUrl={null} title="فيديو" className="h-full w-full rounded-2xl border border-[#08467d]/35 shadow-2xl" /> : <iframe src={embedUrl(content.mediaUrl)} title="فيديو" className="h-full w-full rounded-2xl border border-[#08467d]/35 bg-black shadow-2xl" allow="autoplay; encrypted-media" allowFullScreen /> : <div className="flex h-full w-full items-center justify-center rounded-2xl border border-dashed border-[#08467d]/45 bg-[#08467d]/[0.1] text-[#f8ca14]"><span className="text-center"><PlayCircle className="mx-auto" size={36} /><span className="mt-2 block text-xs font-black">اختر فيديو</span></span></div>;
    }
  ...
  const renderItem = (item: CanvasItem) => {
    ...
    return <div key={item.elementId} data-freeform-id={item.elementId} style={style} onPointerDown={(event) => startInteraction(event, item, "move")} onClick={(event) => { event.stopPropagation(); if (isEditing) setSelectedId(item.elementId); }} className={`absolute -translate-x-1/2 -translate-y-1/2 select-none ${isEditing ? "touch-none cursor-grab active:cursor-grabbing" : ""} ${interaction?.elementId === item.elementId ? "opacity-90" : ""}`}>
      ...
      {isEditing && isSelected ? <>
        <div className="pointer-events-none absolute -inset-1 rounded-[18px] border-2 border-[#f8ca14] shadow-[0_0_0_4px_rgba(248,202,20,.18)]" />
        <div className="pointer-events-none absolute -top-8 left-1/2 flex -translate-x-1/2 items-center gap-1 whitespace-nowrap rounded-lg bg-[#08467d] px-2 py-1 text-[10px] font-black text-white shadow-lg"><Move size={11} />اسحب للتحريك</div>
        {(["nw", "ne", "sw", "se"] as ResizeCorner[]).map((corner) => <button key={corner} type="button" aria-label="تغيير حجم العنصر" onPointerDown={(event) => startInteraction(event, item, "resize", corner)} className={`absolute z-20 h-3.5 w-3.5 rounded-[4px] border-2 border-white bg-[#08467d] shadow ${corner === "nw" ? "-left-2 -top-2 cursor-nwse-resize" : corner === "ne" ? "-right-2 -top-2 cursor-nesw-resize" : corner === "sw" ? "-bottom-2 -left-2 cursor-nesw-resize" : "-bottom-2 -right-2 cursor-nwse-resize"}`} />)}
      </> : null}
    </div>;
  };
  ...
  return <section className="relative border-y border-amber-400/15 bg-[#090d15] py-10" dir="rtl">
    <div className="container">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        ...
        {isEditing ? <div className="flex items-center gap-2"><button type="button" onClick={() => setShowGrid((value) => !value)} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-black transition ${showGrid ? "border-[#08467d]/60 bg-[#08467d]/25 text-white" : "border-slate-700 text-slate-400"}`}><Grid2X2 size={15} />شبكة</button><button type="button" onClick={() => setSnapEnabled((value) => !value)} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-black transition ${snapEnabled ? "border-amber-300/60 bg-amber-400/15 text-amber-100" : "border-slate-700 text-slate-400"}`}><MousePointer2 size={15} />محاذاة ذكية</button></div> : null}
      </div>
      ...
      <div className={`grid gap-4 ${isEditing ? "xl:grid-cols-[246px_minmax(0,1fr)_292px]" : "grid-cols-1"}`}>
        ...
        <div className="order-1 min-w-0 xl:order-2">
          <div className="mb-3 flex items-center justify-between rounded-2xl border border-white/[0.08] bg-[#101722] px-3 py-2.5"><div className="flex items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-lg bg-[#08467d]/20 text-[#f8ca14]"><MousePointer2 size={15} /></span><span className="text-xs font-black text-slate-200">سطح المكتب</span></div><div className="flex items-center gap-2 text-[10px] font-bold text-slate-500"><span className="rounded-full bg-amber-400/10 px-2 py-1 text-amber-200">{displayItems.length} عنصر</span>{isEditing ? <span>{interaction ? "جارٍ التحريك…" : "اضغط واسحب"}</span> : null}</div></div>
          <div ref={canvasRef} onPointerMove={onCanvasPointerMove} onPointerUp={onCanvasPointerUp} onPointerCancel={onCanvasPointerUp} onPointerLeave={(event) => { if (event.buttons === 0) onCanvasPointerUp(); }} onClick={() => isEditing && setSelectedId(null)} onDragOver={onCanvasDragOver} onDragLeave={() => setDropPreview(null)} onDrop={onCanvasDrop} className={`relative aspect-[16/10] min-h-[570px] overflow-hidden rounded-[28px] border bg-[#0a0e16] shadow-[0_24px_80px_rgba(0,0,0,.38)] ${isEditing ? "border-[#08467d]/40" : "border-amber-300/20"} ${showGrid && isEditing ? "[background-image:linear-gradient(rgba(148,163,184,.08)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,.08)_1px,transparent_1px),radial-gradient(circle_at_50%_40%,rgba(251,191,36,.09),transparent_48%)] [background-size:32px_32px,32px_32px,auto]" : "bg-[radial-gradient(circle_at_50%_40%,rgba(251,191,36,.09),transparent_48%)]"}`}>
            {isEditing ? <><div className="pointer-events-none absolute inset-y-0 left-[8%] border-l border-dashed border-amber-200/15" /><div className="pointer-events-none absolute inset-y-0 left-1/2 border-l border-dashed border-[#08467d]/30" /><div className="pointer-events-none absolute inset-y-0 right-[8%] border-l border-dashed border-amber-200/15" /><div className="pointer-events-none absolute inset-x-0 top-1/2 border-t border-dashed border-[#08467d]/30" /></> : null}
            {guides.x !== undefined ? <div className="pointer-events-none absolute inset-y-0 z-40 border-l-2 border-[#f8ca14]" style={{ left: `${guides.x / 10}%` }} /> : null}
            {guides.y !== undefined ? <div className="pointer-events-none absolute inset-x-0 z-40 border-t-2 border-[#f8ca14]" style={{ top: `${guides.y / 10}%` }} /> : null}
          ...
          {isEditing ? <div className="mt-3 flex flex-wrap items-center justify-between gap-2 px-1 text-[10px] text-slate-500"><span>الخط الذهبي = محاذاة ذكية. اضغط <kbd className="rounded bg-white/10 px-1.5 py-0.5 text-slate-300">Shift</kbd> أثناء تحجيم الوسائط للتحجيم الحر.</span><span>المسودة لا تظهر للزوار قبل النشر.</span></div> : null}
        </div>

        {isEditing ? <aside className="order-3 max-h-[760px] overflow-y-auto rounded-3xl border border-white/[0.09] bg-[#101722] p-4">
          {selected ? <>
            <div className="flex items-start justify-between border-b border-white/[0.08] pb-3"><div><div className="text-[10px] font-black text-[#f8ca14]">المفتش الذكي</div><h3 className="mt-1 text-sm font-black text-white">{selected.elementType === "text" ? "نص" : selected.elementType === "image" ? "صورة" : selected.elementType === "video" ? "فيديو" : selected.elementType === "icon" ? "أيقونة" : "زر"} · {selected.preset}</h3></div><button type="button" onClick={() => setSelectedId(null)} className="rounded-lg p-1.5 text-slate-400 hover:bg-white/[0.08] hover:text-white"><X size={16} /></button></div>
            <div className="mt-4 space-y-4">
              ...
              {selected.elementType === "text" || selected.elementType === "button" ? <><label className="block text-[11px] font-bold text-slate-400">المحتوى<textarea value={contentDraft.text || ""} onChange={(event) => applyContent({ text: event.target.value })} onBlur={() => applyContent({}, true)} className="mt-1.5 min-h-24 w-full rounded-xl border border-slate-700 bg-black/20 p-2.5 text-sm leading-6 text-white outline-none focus:border-[#08467d]" /></label>{selected.elementType === "text" ? <label className="block text-[11px] font-bold text-slate-400">محاذاة النص<select value={contentDraft.textAlign || "start"} onChange={(event) => applyContent({ textAlign: event.target.value as TextAlign }, true)} className="mt-1.5 w-full rounded-xl border border-slate-700 bg-black/20 px-2.5 py-2 text-xs text-white outline-none focus:border-[#08467d]"><option value="start">يمين</option><option value="center">وسط</option><option value="end">يسار</option></select></label> : null}</> : null}
              {["image", "video"].includes(selected.elementType) ? <button type="button" onClick={() => setMediaTarget(selected)} className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#08467d]/40 bg-[#08467d]/15 px-3 py-3 text-xs font-black text-white transition hover:bg-[#08467d]/30"><ImageIcon size={15} />اختيار أو رفع {selected.elementType === "image" ? "صورة" : "فيديو"}</button> : null}
          ...
          </> : <div className="grid min-h-[310px] place-items-center text-center"><div><span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-[#08467d]/20 text-[#f8ca14]"><Move size={22} /></span><h3 className="mt-3 text-sm font-black text-slate-100">اختر عنصراً من القماش</h3><p className="mx-auto mt-1 max-w-[210px] text-xs leading-6 text-slate-500">ستظهر أدوات الحجم، المكان، الرابط والوسائط هنا.</p></div></div>}
        </aside> : null}
      </div>
    </div>
...
function NumberField({ label, value, onChange, onCommit }: { label: string; value: number; onChange: (value: number) => void; onCommit: () => void }) {
  return <label className="text-[10px] font-bold text-slate-400">{label}<input type="number" value={value} min={0} max={1000} onChange={(event) => onChange(Number(event.target.value) || 0)} onBlur={onCommit} className="mt-1 block w-full rounded-lg border border-slate-700 bg-black/20 px-2 py-1.5 text-xs font-black text-slate-100 outline-none focus:border-[#08467d]" /></label>;
}

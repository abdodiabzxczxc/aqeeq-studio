  ...
  return (
    <section className="border-b border-white/[0.08] bg-black/15 px-3 pb-3 pt-2" dir="rtl" aria-label="التحكم المباشر بالطبقة">
      <header className="flex items-center justify-between gap-3">
        <div>
          <div className="text-[10px] font-black text-sky-200">٢. عدّل العنصر المحدد</div>
      ...
      <div className="mt-2 grid grid-cols-4 gap-1.5 sm:grid-cols-8">
        ...
        <label className="relative grid h-9 place-items-center rounded-xl border border-white/[0.1] bg-[#101520] text-sky-200" title="لون الخلفية"><span className="h-3.5 w-3.5 rounded border border-white/35" style={{ background: colors.background }} /><input type="color" value={colors.background} onChange={(event) => onDraftChange({ bgColor: event.target.value })} className="absolute inset-0 cursor-pointer opacity-0" /></label>
        ...
        <button onClick={() => onDraftChange({ layerZIndex: draft.layerZIndex + 1 })} title="إلى الأمام" className="grid h-9 place-items-center rounded-xl border border-white/[0.1] bg-[#101520] text-slate-200 hover:border-sky-300 hover:text-sky-100"><ArrowUpToLine size={15} /></button>
        <button onClick={() => onDraftChange({ layerZIndex: draft.layerZIndex - 1 })} title="إلى الخلف" className="grid h-9 place-items-center rounded-xl border border-white/[0.1] bg-[#101520] text-slate-200 hover:border-sky-300 hover:text-sky-100"><ArrowDownToLine size={15} /></button>
        <button onClick={onOpenEffects} title="تأثيرات الطبقة" className="grid h-9 place-items-center rounded-xl border border-fuchsia-300/35 bg-fuchsia-300/10 text-fuchsia-100 hover:bg-fuchsia-300 hover:text-fuchsia-950"><Sparkles size={15} /></button>
        ...
        <button onClick={onDelete} title="نقل إلى السلة" className="grid h-9 place-items-center rounded-xl border border-rose-300/35 bg-rose-300/10 text-rose-100 hover:bg-rose-300 hover:text-rose-950"><Trash2 size={15} /></button>
      </div>

...
export function AqeeqMagneticCursor() {
  ...
  // Outer ring — follows with lag (springy)
  ...
  useEffect(() => {
    ...
    const onOver = (e: MouseEvent) => {
      ...
      if (el) {
        ...
        setCursorLabel(el.dataset.cursorText ?? "");
      } else {
  ...
  return (
    <>
      ...
      >
        <motion.div
          className="w-full h-full rounded-full border flex items-center justify-center overflow-hidden"
          animate={{
            borderColor: cursorState === "hover" ? "rgba(248,202,20,0.85)" : "rgba(255,255,255,0.55)",
            backgroundColor: cursorState === "hover" ? "rgba(248,202,20,0.06)" : "transparent",
          }}
          ...
          <AnimatePresence>
            {cursorLabel && (
              <motion.span
                ...
                className="text-[8px] font-black text-[#f8ca14] tracking-widest uppercase text-center leading-tight px-1"
              >
                {cursorLabel}
              </motion.span>
            )}
      ...
      >
        <motion.div
          className="rounded-full bg-[#f8ca14]"
          animate={{

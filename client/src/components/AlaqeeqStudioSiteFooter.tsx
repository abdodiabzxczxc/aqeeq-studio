  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1380px] 2xl:max-w-[1560px] px-4 sm:px-6 md:px-8 pt-8 pb-28 md:pb-10">
        ...
        {/* 3. Centered Copyright Line & Policy Links */}
        <div className="mt-4 pt-3 space-y-2 text-center text-[11px] font-bold">
          {/* Policy Links */}
          {(orchestration?.footer?.privacyPolicyText || orchestration?.footer?.termsText) && (
            <div className={`flex items-center justify-center gap-3 text-[11px] font-semibold ${
              dark ? "text-slate-400" : "text-slate-500"
            }`}>
              {orchestration?.footer?.privacyPolicyText && (
                <a
                  href={orchestration.footer.privacyPolicyUrl || "/about#privacy"}
                  onClick={(e) => {
                    const url = orchestration?.footer?.privacyPolicyUrl || "/about#privacy";
                    if (url.startsWith("/")) {
                      e.preventDefault();
                      navigate(url);
                    }
                  }}
                  className="hover:underline transition hover:text-amber-500 cursor-pointer"
                >
                  {orchestration.footer.privacyPolicyText}
                </a>
              )}
              ...
              {orchestration?.footer?.termsText && (
                <a
                  href={orchestration.footer.termsUrl || "/admissions#terms"}
                  onClick={(e) => {
                    const url = orchestration?.footer?.termsUrl || "/admissions#terms";
                    if (url.startsWith("/")) {
                      e.preventDefault();
                      navigate(url);
                    }
          ...
          {/* Centered Copyright & Clickable Location */}

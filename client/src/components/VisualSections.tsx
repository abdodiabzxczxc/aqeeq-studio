...
function SectionContentEditor({ section, pagePath, onClose }: { section: BuilderSection; pagePath: string; onClose: () => void }) {
  ...
  return <><aside className="fixed bottom-5 right-5 top-24 z-[140] flex w-[min(430px,calc(100vw-40px))] flex-col overflow-hidden rounded-3xl border border-[#08467d]/40 bg-[#111722]/[.98] shadow-[0_30px_90px_rgba(0,0,0,.68)] backdrop-blur-xl" dir="rtl">
    <header className="flex items-start justify-between border-b border-white/[0.08] p-4"><div><div className="text-[10px] font-black tracking-[0.15em] text-[#f8ca14]">EDIT THIS BLOCK</div><h2 className="mt-1 text-base font-black text-white">تعديل محتوى القالب</h2><p className="mt-1 text-[11px] leading-5 text-slate-500">كل نص ووسيط ورابط في هذا القالب قابل للتعديل.</p></div><button type="button" onClick={onClose} className="rounded-xl p-2 text-slate-400 transition hover:bg-white/[0.07] hover:text-white"><X size={17} /></button></header>
    <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
      ...
      {section.sectionType === "video" || draft.videoUrl !== undefined ? <div className="space-y-3 rounded-2xl border border-[#08467d]/30 bg-[#08467d]/10 p-3"><div className="text-[11px] font-black text-amber-100">الفيديو</div><button type="button" onClick={() => setMediaField("videoUrl")} className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#08467d]/40 bg-[#08467d]/20 px-3 py-2.5 text-xs font-black text-[#f8ca14] transition hover:bg-[#08467d]/30"><Video size={15} />اختيار أو رفع فيديو</button><TextField label="رابط فيديو أو YouTube / Vimeo" value={draft.videoUrl || ""} placeholder="https://..." onChange={(videoUrl) => update({ videoUrl })} /><TextField label="رابط عند النقر على الفيديو" value={draft.videoHref || ""} placeholder="/page/... أو https://..." onChange={(videoHref) => update({ videoHref })} icon={<Link2 size={13} />} /></div> : null}
      {draft.builderElement === "icon" ? <div className="space-y-3 rounded-2xl border border-amber-400/20 bg-amber-400/[0.035] p-3"><div className="text-[11px] font-black text-amber-100">الأيقونة التفاعلية</div><label className="block text-[11px] font-bold text-slate-400">الرمز<select value={draft.iconName || "sparkles"} onChange={(event) => update({ iconName: event.target.value as NonNullable<SectionConfig["iconName"]> })} className="mt-1.5 w-full rounded-xl border border-slate-700 bg-black/20 px-3 py-2.5 text-xs text-white outline-none focus:border-amber-400"><option value="sparkles">احتفال</option><option value="star">نجمة</option><option value="calendar">موعد</option><option value="location">موقع</option><option value="guests">ضيوف</option><option value="heart">قلب</option></select></label><TextField label="رابط الأيقونة أو الصفحة" value={draft.iconHref || ""} placeholder="/page/... أو https://..." onChange={(iconHref) => update({ iconHref })} icon={<Link2 size={13} />} /></div> : null}
...
function TextField({ label, value, onChange, placeholder = "", icon }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; icon?: React.ReactNode }) {
  return <label className="block text-[11px] font-bold text-slate-400">{label}<div className="relative mt-1.5">{icon ? <span className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-amber-300">{icon}</span> : null}<input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className={`w-full rounded-xl border border-slate-700 bg-black/20 py-2.5 text-xs text-white outline-none transition focus:border-amber-400 ${icon ? "pr-8 pl-3" : "px-3"}`} /></div></label>;
}

function TextAreaField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className="block text-[11px] font-bold text-slate-400">{label}<textarea value={value} onChange={(event) => onChange(event.target.value)} className="mt-1.5 min-h-20 w-full rounded-xl border border-slate-700 bg-black/20 p-3 text-xs leading-6 text-white outline-none transition focus:border-amber-400" /></label>;
}
...
function BuilderSectionView({ section, pagePath }: { section: BuilderSection; pagePath: string }) {
  ...
  if (section.sectionType === "video") return <VisualEditable id={section.sectionId} tag="section-block" label={`قسم فيديو: ${config.title || "جديد"}`} as="section" className="bg-[#0b0f18]"><div className={`${shell} grid items-center gap-7 lg:grid-cols-[.8fr_1.2fr]`}><div><div className="inline-flex items-center gap-2 text-xs font-black text-[#f8ca14]"><PlayCircle size={16} />فيديو</div><h2 className="mt-3 text-3xl font-black text-amber-100">{config.titleHref ? <a href={config.titleHref}>{config.title || "فيديو الفعالية"}</a> : config.title || "فيديو الفعالية"}</h2>{config.body ? <p className="mt-4 text-sm leading-7 text-slate-400">{config.bodyHref ? <a href={config.bodyHref}>{config.body}</a> : config.body}</p> : null}</div><VisualVideo id={`${section.sectionId}-video`} label={`فيديو قسم ${config.title || "جديد"}`} src={config.videoUrl} linkUrl={config.videoHref} title={config.title || "فيديو"} className="aspect-video overflow-hidden rounded-3xl border border-[#08467d]/30 bg-black" /></div></VisualEditable>;

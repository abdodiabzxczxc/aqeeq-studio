...
export function HeaderDockNav({ items, dark, onNavigate }: HeaderDockNavProps) {
  ...
  const livePreviews = useMemo<Record<string, PagePreviewMetadata>>(() => {
    ...
    return {
      ...
      about: {
        ...
        heroTitleLine1: "مدارس العقيق",
        heroTitleLine2: "الأهلية والدولية.",
      ...
      accreditations: {
        ...
        heroTitleLine1: "اعتمادات دولية مرموقة",
        heroTitleLine2: "ومراكز اختبارات معتمدة.",
      ...
      admissions: {
        ...
        heroTitleLine1: "استثمر في مستقبل أبنائك",
        heroTitleLine2: "في صرح العقيق الرائد.",
  ...
  const handleMouseEnterItem = (key: string) => {
    ...
    const itemEl = itemRefs.current[key];
    const navEl = navRef.current;
    if (itemEl && navEl) {
      const itemRect = itemEl.getBoundingClientRect();
      const navRect = navEl.getBoundingClientRect();
